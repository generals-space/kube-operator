...
func (r *Request) Watch() (watch.Interface, error) {
	...
	// We specifically don't want to rate limit watches, 
	// so we don't use r.rateLimiter here.
	if r.err != nil {
		return nil, r.err
	}

	///////// 重新封装 watch 形式的 request.
	url := r.URL().String()
	// Watch() 的主调函数一般使用 Get() 类型
	...
	if err != nil {
		// The watch stream mechanism handles many common partial data errors, 
		// so closed connections can be retried in many cases.
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf(
			"for request %s, got status: %v", url, resp.StatusCode,
		)
	}
	...
	if err != nil {
		klog.V(4).Infof(
			"Unexpected content type from the server: %q: %v", contentType, err,
		)
	}
	...
	return watch.NewStreamWatcher(
		...
		errors.NewClientErrorReporter(
			http.StatusInternalServerError, r.verb, "ClientWatchDecoding",
		),
	), nil
}
...
// Returns io.ReadCloser which could be used for streaming of the response, 
// or an error Any non-2xx http status code causes an error. 
// If we get a non-2xx code, we try to convert the body into an APIStatus object.
// If we can, we return that as an error. 
// Otherwise, we create an error that lists the http status and the content of the response.
...
func (r *Request) transformResponse(resp *http.Response, req *http.Request) Result {
	...
	if resp.Body != nil {
		...
		case http2.StreamError:
			...
			klog.V(2).Infof(
				"Stream error %#v when reading response body, may be caused by closed connection.", err,
			)
			streamErr := fmt.Errorf(
				"stream error when reading response body, may be caused by closed connection. Please retry. Original error: %v", err,
			)
			return Result{
				err: streamErr,
			}
		default:
			...
			unexpectedErr := fmt.Errorf(
				"unexpected error when reading response body. Please retry. Original error: %v", err,
			)
			return Result{
				err: unexpectedErr,
			}
	...
	if len(contentType) > 0 {
		...
		// decoder 为 apimachinery/pkg/runtime/helper.go -> WithoutVersionDecoder{} 对象
...
// Result Request.Do() 方法将返回 Result{} 对象.
//
...
// If the returned object is of type Status and has .Status != StatusSuccess,
// the additional information in Status will be used to enrich the error.
func (r Result) Into(obj runtime.Object) error {
	...
	// 这里的 Decode() 最终调用的是 pkg/runtime/serializer/recognizer/recognizer.go ->
	// decode{}.Decode() 方法
	// 这一步解析的过程其实是由 apimachinery 库完成的.
	out, _, err := r.decoder.Decode(r.body, nil, obj)
	if err != nil || out == obj {
		return err
	}
	// if a different object is returned, 
	// see if it is Status and avoid double decoding the object.
	switch t := out.(type) {
	case *metav1.Status:
		// any status besides StatusSuccess is considered an error.
		if t.Status != metav1.StatusSuccess {
...
func (r Result) Error() error {
	// if we have received an unexpected server error, 
	// and we have a body and decoder, we can try to extract a Status object.
