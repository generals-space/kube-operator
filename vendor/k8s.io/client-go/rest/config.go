...
type Config struct {
	...
	// UserAgent 请求 apiserver 的 user-agent 请求头, 存在默认值
	//
...
type ContentConfig struct {
	...
	// NegotiatedSerializer 看了下 core, apps, batch 3个资源组的值, 使用的都是同一个.
	// 
...
// RESTClientFor 根据 rest.Config{} 配置构建 rest.Client 对象.
//
// caller: 
// 	1. dynamic/simple.go -> NewForConfig()
// 	2. kubernetes/typed/core/v1/core_client.go -> NewForConfig()
//
...
func RESTClientFor(config *Config) (*RESTClient, error) {
	...
	// 不同类型的资源, gv 和 Negotiator 信息是不同的, 一般按资源组进行划分, 如 corev1, appsv1 等.
...
// caller: discovery/discovery_client.go -> NewDiscoveryClientForConfig()
...
// InClusterConfig 读取 Pod 内部挂载的 ServiceAccount 信息, 将其转换为 rest.Config{} 对象.
//
...
// AnonymousClientConfig returns a copy of the given config with
// all user credentials (cert/key, bearer token, and username/password) 
// and custom transports (WrapTransport, Transport) removed
