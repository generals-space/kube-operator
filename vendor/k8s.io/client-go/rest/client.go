...
//
// 在 rest/config.go -> RESTClientFor() 中被初始化
//
...
type ClientContentConfig struct {
	...
	// AcceptContentTypes is not set, 
	// and as the default content type on any object sent to the server. 
	// If not set, "application/json" is used.
	ContentType string
	// GroupVersion is the API version to talk to. Must be provided when initializing
	// a RESTClient directly. When initializing a Client, will be set with the default
	// code version. This is used as the default group version for VersionedParams.
	GroupVersion schema.GroupVersion
	// Negotiator 为 apimachinery/pkg/runtime/negotiate.go -> clientNegotiator{} 对象
...
// NewRESTClient 对 baseURL 参数进行格式化, 然后将传入的参数封装成 RESTClient{} 对象.
//
// 	@param client: 由主调函数设置 transport 与超时时间.
//
// caller: 
//	1. rest/config.go -> RESTClientFor()
//	2. rest/config.go -> UnversionedRESTClientFor()
//
// NewRESTClient creates a new RESTClient. 
// This client performs generic REST functions
// such as Get, Put, Post, and Delete on specified paths.
//
func NewRESTClient(
	baseURL *url.URL, versionedAPIPath string, 
	...
	base.RawQuery = "" // url 中的参数列表, 如 ?a=1&b=2
	base.Fragment = "" // url 中的井号部分, 如 #title1
