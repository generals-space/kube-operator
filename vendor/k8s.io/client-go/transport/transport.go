...
// New 根据 transport.Config{} 配置, 创建 http.RoundTripper{} 对象.
// 在该流程中, 会根据 transport.Config{} 中的字段, 选择合适的认证手段(如 basic auth, bearer token等)
//
// caller: rest/transport.go -> TransportFor()
//
...
// TLSConfigFor 将 transport.Config.TLSConfig{} 对象转换为标准库 crypto/tls 中的 Config{} 对象.
//
...
func TLSConfigFor(c *Config) (*tls.Config, error) {
	...
	if c.HasCertAuth() || c.HasCertCallback() {
		tlsConfig.GetClientCertificate = func(*tls.CertificateRequestInfo) (*tls.Certificate, error) {
			// Note: static key/cert data always take precedence over cert callback.
			if staticCert != nil {
...
// loadTLSFiles 读取 ca.crt, client.crt 与 client.key 文件, 确保ta们存在于 config.TLS 对象中.
//
// loadTLSFiles copies the data from the CertFile, KeyFile, and CAFile fields 
// into the CertData, KeyData, and CAFile fields, or returns an error. 
...
// dataFromSliceOrFile 判断 data 是否有内容, 如果为空则读取 file 文件中的内容并返回.
//
...
// rootCertPool returns nil if caData is empty. 
// When passed along, this will mean "use system CAs".
// When caData is not empty, it will be the ONLY information used in the CertPool.
func rootCertPool(caData []byte) *x509.CertPool {
	// What we really want is a copy of x509.systemRootsPool, but that isn't exposed. 
	// It's difficult to build (see the go code for a look at the platform specific insanity), 
	// so we'll use the fact that RootCAs == nil gives us the system values
