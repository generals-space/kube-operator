...
// HTTPWrappersForConfig 根据 transport.Config{} 对象中存在的认证字段(包括 basicAuth, bearer token)
// 选择合适的认证手段, 对传入的 RoundTripper 对象进行进一步封装.
//
...
// HTTP2 clients). 
// Pure HTTP clients should use the RoundTripper returned from New.
