...
// 由 client-go/rest/watch/decoder.go -> Decoder{} 结构体实现
...
// 实现了 pkg/watch/watch.go -> Interface 接口
//
...
// caller: client-go/rest/request.go -> Request.Watch()
//
	...
	for {
		...
		if err != nil {
			...
			case io.ErrUnexpectedEOF:
				klog.V(1).Infof(
					"Unexpected EOF during watch stream event decoding: %v", err,
				)
			default:
				if net.IsProbableEOF(err) {
					klog.V(5).Infof(
						"Unable to decode an event from the watch stream: %v", err,
					)
				} else {
					sw.result <- Event{
						Type:   Error,
						Object: sw.reporter.AsObject(fmt.Errorf(
							"unable to decode an event from the watch stream: %v", err,
						)),
					}
				}
