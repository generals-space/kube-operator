...
// ParseResourceArg takes the common style of string which may be either 
// `resource.group.com` or `resource.version.group.com`
// and parses it out into both possibilities. 
// This code takes no responsibility for knowing which representation was intended
// but with a knowledge of all GroupVersions, calling code can take a very good guess. 
// If there are only two segments, then `*GroupVersionResource` is nil.
...
// ParseGroupVersion 将 groupVersion 字符串转换成 GroupVersion 对象.
//
// 	@param gv: 各资源 yaml 内容中的 apiVersion 字段, 如 v1(Pod), apps/v1(DaemonSet) 等.
//
// caller: pkg/runtime/serializer/json/meta.go -> SimpleMetaFactory.Interpret()
//
// ParseGroupVersion turns "group/version" string into a GroupVersion struct. 
// It reports error if it cannot parse the string.
func ParseGroupVersion(gv string) (GroupVersion, error) {
	// this can be the internal version for the legacy kube types
	// TODO once we've cleared the last uses as strings, 
