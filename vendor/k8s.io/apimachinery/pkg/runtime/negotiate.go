...
// clientNegotiator 实现了 pkg/runtime/interfaces.go -> ClientNegotiator 接口
// 
// encode, decode 是各 Group 对象(如 core/v1, apps/v1 等)
type clientNegotiator struct {
	serializer     NegotiatedSerializer
	encode, decode GroupVersioner
}

// 	@param serizlizer: apimachinery/pkg/runtime/serializer.NewCodecFactory().WithoutConversion()
// 	@param gv: 各资源组的 GV 信息, 如 schema.GroupVersion{Group: "apps", Version: "v1"}
//
...
func NewClientNegotiator(
	serializer NegotiatedSerializer, gv schema.GroupVersion,
) ClientNegotiator {
...
// Decoder 根据客户端传入的可接受的 contentType 类型, 选择合适的序列化器对象.
// 可选的序列化器一般有3个, json,yaml,protobuf
//
// 	@param contentType: application/json, application/protobuf 等, 一般是前者
// 	@param params: 
//
// caller: 
// 	1. client-go/rest/request.go -> Request.transformResponse() 
// 		得到 apiserver 的响应后, 获取 Decoder 对响应结果进行解析.
func (n *clientNegotiator) Decoder(
