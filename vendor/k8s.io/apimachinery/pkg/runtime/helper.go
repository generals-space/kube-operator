...
// WithoutVersionDecoder 比如 json,yaml,protobuf 等序列化器对象(既可以编码, 也可以解码).
//
// 在 pkg/runtime/serializer/codec_factory.go -> WithoutConversionCodecFactory.DecoderToVersion() 中初始化.
//
// WithoutVersionDecoder clears the group version kind of a deserialized object.
type WithoutVersionDecoder struct {
	// 这个 Decoder 对象被赋值为 pkg/runtime/serializer/recognizer/recognizer.go -> decoder{}
	Decoder
}

// Decode 调用 decoder 解码器, 将传入的 data 数据反序列化成 obj 对象并将其返回.
//
// 	@param data: 待格式化的 byte 数据信息
// 	@param defaults:
// 	@param into: 如果 into 不为 nil, 则会根据 into 的类型反格式化, 并作为 obj 对象返回.
//
// caller: 
// 	1. client-go/rest/request.go -> Result.Into()
// 	2. pkg/runtime/codec.go -> Decode()
//
// Decode does not do conversion. It removes the gvk during deserialization.
func (d WithoutVersionDecoder) Decode(
	data []byte, defaults *schema.GroupVersionKind, into Object,
) (Object, *schema.GroupVersionKind, error) {
	// 实际调用了 pkg/runtime/serializer/json/serializer.go -> Serializer.Decode()
