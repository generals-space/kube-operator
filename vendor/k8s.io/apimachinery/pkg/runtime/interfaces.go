...
// Decoder 实现列表
// 	1. pkg/runtime/helper.go -> WithoutVersionDecoder{} (不够典型)
// 	2. pkg/runtime/serializer/json/serializer.go -> Serializer{} (json序列化器)
//
...
// NegotiatedSerializer 由 pkg/runtime/serializer/codec_factory.go -> CodecFactory{} 实现
//
// NegotiatedSerializer is an interface used for obtaining 
// encoders, decoders, and serializers for multiple supported media types. 
// This would commonly be accepted by a server component
// that performs HTTP content negotiation to accept multiple formats.
type NegotiatedSerializer interface {
	// SupportedMediaTypes 比如 application/json, protobuf 什么的.
	//
	// pkg/runtime/serializer/codec_factory.go -> CodecFactory.accepts 成员
	// 该成员在 pkg/runtime/serializer/codec_factory.go -> newCodecFactory() 中初始化.
	//
	// SupportedMediaTypes is the media types supported for reading and 
	// writing single objects.
...
// ClientNegotiator 由 pkg/runtime/negotiate.go -> clientNegotiator{} 结构体实现
//
...
// 由 pkg/runtime/scheme.go -> Scheme{} 结构体实现
//
...
type ObjectTyper interface {
	...
	//
	// ObjectKinds returns the all possible group,version,kind of the provided object, 
	// true if the object is unversioned, or an error if the object is not recognized
...
// EquivalentResourceMapper provides information about resources 
// that address the same underlying data as a specified resource
