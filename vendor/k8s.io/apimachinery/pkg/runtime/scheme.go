...
// 实现了 
// 	1. pkg/runtime/interfaces.go -> ObjectCreater 接口
// 	2. pkg/runtime/interfaces.go -> ObjectTyper 接口
//
// Scheme defines methods for serializing and deserializing API objects, 
// a type registry for converting group, version, and kind information to and from
// Go schemas, and mappings between Go schemas of different versions. 
// A scheme is the foundation for a versioned API and versioned configuration over time.
//
// In a Scheme, a Type is a particular Go struct, 
// a Version is a point-in-time identifier for a particular representation of that Type
// (typically backwards compatible), 
// a Kind is the unique name for that Type within the Version, 
// and a Group identifies a set of Versions, Kinds, and Types that evolve over time. 
// An Unversioned Type is one that is not yet formally bound to a type and
// is promised to be backwards compatible 
// (effectively a "v1" of a Type that does not expect to break in the future).
//
// Schemes are not expected to change at runtime and are only threadsafe after
// registration is complete.
type Scheme struct {
	// gvkToType/typeToGVK 是两个互为 k/v 的 map 对象...

	// gvkToType 这个字典中存储了当前版本 client-go 已知的所有的资源类型.
	// key 为 GVK 对象, 如 GVK{Group: "", Version: "v1", Kind: "Pod"}, 即 Pod{} 类型.
	//
	// versionMap allows one to figure out the go type of an object with
	// the given version and name.
	gvkToType map[schema.GroupVersionKind]reflect.Type
	
	// typeToGVK key一般为 *corev1.Pod{}, *apps.Deployment{} 等.
	// 在 Scheme.AddUnversionedTypes() -> Scheme.AddKnownTypeWithName() 方法中被初始化, 
	// 将已知的资源类型添加到此map中.
	//
	// 在 client-go 在调用 Into(runtime.Object)时, 会在这个 map 进行查询, 
	// 以得到 runtime.Object 实际的 GVK 对象.
	//
	// typeToGroupVersion allows one to find metadata for a given go object.
	// The reflect.Type we index by should *not* be a pointer.
	typeToGVK map[reflect.Type][]schema.GroupVersionKind

	// unversionedTypes ta的 key 与 typeToGVK 成员属性的 key 相同.
	// 在 Scheme.AddUnversionedTypes() 方法中被初始化
	//
	// unversionedTypes are transformed without conversion in ConvertToVersion.
	unversionedTypes map[reflect.Type]schema.GroupVersionKind

	// unversionedKinds ta的 **value** 与 typeToGVK 成员属性的 key 相同.
	// 在 Scheme.AddUnversionedTypes() 方法中被初始化
	//
	// unversionedKinds are the names of kinds that can be created 
	// in the context of any group or version
	...
	// observedVersions 目前已知资源组的不同 GV 列表, 比如
	// GV{Group: "", Version: "v1"}
	// GV{Group: "", Version: "v1beta1"}
	//
	// observedVersions keeps track of the order we've seen versions during type registration
	observedVersions []schema.GroupVersion

	// schemeName is the name of this scheme. 
	// If you don't specify a name, the stack of the NewScheme caller will be used.
...
// caller: 
// 	1. client-go/kubernetes/scheme/register.go -> var Scheme = runtime.NewScheme()
//
...
func NewScheme() *Scheme {
	...
	utilruntime.Must(
		s.RegisterInputDefaults(
			&map[string][]string{}, 
			JSONKeyMapper, 
			conversion.AllowDifferentFieldTypeNames|conversion.IgnoreMissingFields,
		),
	)
	utilruntime.Must(
		s.RegisterInputDefaults(
...
// caller: 在 NewScheme() 中将本函数赋值为 Converter 对象的成员属性 nameFunc.
// 	1. pkg/conversion/converter.go -> Converter.defaultConvert()
//
// nameFunc returns the name of the type that we wish to use to determine 
// when two types attempt a conversion. 
// Defaults to the go name of the type if the type is not registered.
func (s *Scheme) nameFunc(t reflect.Type) string {
	...
	for _, gvk := range gvks {
		...
		internalGV.Version = APIVersionInternal 
...
// AddUnversionedTypes 解析传入的 types 列表, 使用 reflect 包得到ta们的对象类型信息, 
// 并将ta们注册到 Scheme.unversionedKinds 字典中.
//
// **同时** 调用 AddKnownTypes() 将ta们注册到 Scheme 的 gvkToType 和 typeToGVK 字典中.
//
// AddUnversionedTypes registers the provided types as "unversioned", 
// which means that they follow special rules.
// Whenever an object of this type is serialized, 
// it is serialized with the provided group version and is not converted. 
// Thus unversioned objects are expected to remain backwards compatible forever, 
// as if they were in an API group and version that would never be updated.
//
// TODO: there is discussion about removing unversioned and replacing it with objects
// that are manifest into every version with particular schemas. 
// Resolve this method at that point.
func (s *Scheme) AddUnversionedTypes(version schema.GroupVersion, types ...Object) {
	...
	for _, obj := range types {
		...
		if old, ok := s.unversionedKinds[gvk.Kind]; ok && t != old {
			panic(fmt.Sprintf(
				"%v.%v has already been registered as unversioned kind %q - kind name must be unique in scheme %q", 
...
// AddKnownTypes 解析传入的 types 列表, 使用 reflect 包得到ta们的对象类型信息, 
// 然后调用 AddKnownTypeWithName() 方法将ta们注册到 Scheme 的 gvkToType 和 typeToGVK 字典中.
//
// caller:
// 	1. Scheme.AddUnversionedTypes()
//
...
// AddKnownTypeWithName 将传入的 gvk 资源组信息以及 obj.Type(reflect) 对象类型信息, 
// 注册到 Scheme 的 gvkToType 和 typeToGVK 字典中.
//
// caller: 
// 	1. Scheme.AddKnownTypes()
// 	2. pkg/apis/meta/v1/register.go -> AddToGroupVersion()
//
...
func (s *Scheme) AddKnownTypeWithName(gvk schema.GroupVersionKind, obj Object) {
	...
	// 当发现 gvkToType 的 key 中已经存在待注册的 gvk 对象, 但 value 不相符, 表示发生了冲突.
	if oldT, found := s.gvkToType[gvk]; found && oldT != t {
		panic(fmt.Sprintf(
			"Double registration of different types for %v: old=%v.%v, new=%v.%v in scheme %q", 
...
// KnownTypes 与 AllKnownTypes() 方法相比, 本函数接收一个 GV 对象参数, 
// 返回的是该 group 下的所有已知资源.
//
...
// ObjectKinds 返回 obj 对象可能的 GVK 资源类型列表
//
// 	@param obj: 通过 PodApi, NodeApi等得到的 &Pod{}, &Node{} 等对象.
//
// caller: 
// 	1. pkg/runtime/serializer/json/serializer.go -> Serializer.Decode()
// 	2. pkg/runtime/codec.go -> UseOrCreateObject()
//
// ObjectKinds returns all possible group,version,kind of the go object,
// true if the object is considered unversioned, 
...
// Recognizes returns true if the scheme is able to 
// handle the provided group,version,kind of an object.
...
// New 根据传入的 GVK 对象, 为其创建一个完成 runtime.Object 对象(借用 reflect 包实现)
// 如传入 GVK{Group: "", Version: "v1", Kind: "Pod"}, 就返回一个 Pod{} 对象.
//
...
// 	1. NewScheme()
//
// AddConversionFuncs adds functions to the list of conversion functions. 
// The given functions should know how to convert between two of your API objects, 
// or their sub-objects. 
// We deduce how to call these functions from the types of their two parameters; 
// see the comment for Converter.Register.
//
...
// RegisterInputDefaults 直接调用 converter 成员的同名函数...
//
// RegisterInputDefaults sets the provided field mapping function and field matching
// as the defaults for the provided input type.  The fn may be nil, in which case no
// mapping will happen by default. Use this method to register a mechanism for handling
// a specific input type in conversion, such as a map[string]string to structs.
func (s *Scheme) RegisterInputDefaults(
	in interface{}, 
	fn conversion.FieldMappingFunc, 
	defaultFlags conversion.FieldMatchingFlags,
) error {
...
// PrioritizedVersionsAllGroups returns all known versions in their priority order. 
// Groups are random, but versions for a single group are prioritized
...
// addObservedVersion 每一次注册GV对象时, 都要将GV信息保存到 Scheme.observedVersions 列表中.
//
// caller:
// 	1. Scheme.AddUnversionedTypes()
// 	2. Scheme.AddKnownTypes()
// 	3. Scheme.AddKnownTypeWithName()
