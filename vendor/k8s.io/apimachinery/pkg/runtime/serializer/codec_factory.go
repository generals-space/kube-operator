...
// newSerializersForScheme 初始化了 json, yaml, protobuf 3种序列化器对象, 并返回列表.
//
// 	@param scheme: pkg/runtime/scheme.go -> NewScheme()的返回值(几乎所有的调用都使用这个)
//
// caller: NewCodecFactory()
func newSerializersForScheme(
	scheme *runtime.Scheme, mf json.MetaFactory, options CodecFactoryOptions,
) []serializerType {
...
type CodecFactory struct {
	...
	// 一般是 json, yaml, protobuf 这3种类型的序列化器
	serializers []serializerType
	universal   runtime.Decoder
	// accepts newSerializersForScheme() 中定义了3种序列化器对象(json,yaml,protobuf).
...
//
// caller: 
// 	1. client-go/kubernetes/scheme/register.go -> var Codecs = serializer.NewCodecFactory(Scheme)
//
...
func NewCodecFactory(
	scheme *runtime.Scheme, mutators ...CodecFactoryOptionsMutator,
) CodecFactory {
...
//
//
// newCodecFactory is a helper for testing 
// that allows a different metafactory to be specified.
func newCodecFactory(
	scheme *runtime.Scheme, serializers []serializerType,
) CodecFactory {
...
// SupportedMediaTypes json,yaml,protobuf 3种序列化器
...
// 实现了 pkg/runtime/interfaces.go -> Decoder 接口
//
// 在 CodecFactory.WithoutConversion() 中初始化.
//
// WithoutConversionCodecFactory is a CodecFactory that 
// will explicitly ignore requests to perform conversion.
// This wrapper is used while code migrates away from using conversion
//  (such as external clients) and in the future
...
// but does set the group version kind of the object when serialized.
func (f WithoutConversionCodecFactory) EncoderForVersion(
	serializer runtime.Encoder, version runtime.GroupVersioner,
) runtime.Encoder {
...
// caller: pkg/runtime/negotiate.go -> clientNegotiator.Decoder()
func (f WithoutConversionCodecFactory) DecoderToVersion(
	serializer runtime.Decoder, _ runtime.GroupVersioner,
) runtime.Decoder {
