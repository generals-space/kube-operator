...
// UseOrCreateObject 如果 obj 对象为 nil, 则根据 gvk 类型创建一个对应的资源对象.
//
// 	@param t: pkg/runtime/scheme.go -> NewScheme()的返回值(几乎所有的调用都使用这个)
//	@param c: 同 t 参数.
//	@param gvk: rest-client 请求 apiserver 响应体中解析出来的 GVK 资源类型
//	@param obj: rest-client Into(obj) 的 obj 对象, 如 &Pod{}, &Node{} 等(不过应该可以为 nil)
//
// caller: 在处理 apiserver 响应体中的 bytes 数据时被调用, client-go 需要对数据进行反序列化.
// 	1. pkg/runtime/serializer/json/serializer.go -> Serializer.Decode()
// 	2. pkg/runtime/serializer/protobuf/protobuf.go -> unmarshalToObject()
//
// UseOrCreateObject returns obj if the canonical ObjectKind returned by 
// the provided typer matches gvk, 
// or invokes the ObjectCreator to instantiate a new gvk. 
// Returns an error if the typer cannot find the object.
func UseOrCreateObject(
	t ObjectTyper, c ObjectCreater, gvk schema.GroupVersionKind, obj Object,
) (Object, error) {
	if obj != nil {
...
// NoopEncoder converts an Decoder to a Serializer or Codec for code 
// that expects them but only uses decoding.
...
// SerializerInfoForMediaType 从 types 列表选择与 mediaType 匹配的成员并返回.
//
// 	@param types: 
// 	@param mediaType: 比如 application/json
//
// SerializerInfoForMediaType returns the first info in types 
// that has a matching media type (which cannot include media-type parameters), 
// or the first info with an empty media type, or false if no type matches.
func SerializerInfoForMediaType(
	types []SerializerInfo, mediaType string,
) (SerializerInfo, bool) {
	// 先判断类型有没有匹配的
