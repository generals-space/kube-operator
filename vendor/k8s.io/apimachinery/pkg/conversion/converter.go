...
type Converter struct {
	...
	// nameFunc 目前来看, 只有一个取值 pkg/runtime/scheme.go -> Scheme.nameFunc()
	//
...
// NewConverter ...
//
// @param nameFn: 目前来看, 只有一个取值 pkg/runtime/scheme.go -> Scheme.nameFunc()
//
// caller: 
// 	1. pkg/runtime/scheme.go -> NewScheme()
//
...
// 	@param in: 一般是空的类型对象(如 &map[string][]string{}, &url.Values{} 等), 而不是常规对象.
// 	@param fn: pkg/runtime/conversion.go -> JSONKeyMapper()
//
// RegisterInputDefaults registers a field name mapping function,
// used when converting from maps to structs.
// Inputs to the conversion methods are checked for this type and a mapping
// applied automatically if the input matches in.
// A set of default flags for the input conversion
// may also be provided, which will be used when no explicit flags are requested.
func (c *Converter) RegisterInputDefaults(
	in interface{}, fn FieldMappingFunc, defaultFlags FieldMatchingFlags,
) error {
	fv := reflect.ValueOf(in) // 这里的 ValueOf() 主要是为了执行下面的 Type() 函数
	ft := fv.Type()           // 得到参数 in 的类型, 如 int, string, 还可以得到自定义 struct 类型.
	// 由于传入的参数 in 一般为指针类型, Type() 得到的将会是 &url.Values{} 这种, 
	// 而 Kind() 却不一样, 所有指针对象的 Kind() 方法都只能得到 reflect.Ptr.
	// 可以说, 下面这句就是为了判断参数 in 是不是指针类型的.
