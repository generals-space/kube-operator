...
// 	@param obj: 各类型资源的列表对象, 如 PodList{Items:[]} 这种.
//
// caller: 
// 	1. resourceAccessor.Continue()
// 	2. resourceAccessor.SetContinue()
// 	3. client-go/tools/pager/pager.go -> ListPager.List()
//	在开发者执行 informer.Run() 时, 先调用 List() 方法进行全量同步, 得到一个列表, 交由此函数处理.
//
// ListAccessor returns a List interface for the provided object or
// an error if the object does not provide List.
// IMPORTANT: Objects are NOT a superset of lists. 
// Do not use this check to determine whether an object *is* a List.
